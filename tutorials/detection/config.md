# Learn From Config
本教程详细介绍了config中每个参数的实际含义。

```
'name': 'MobileNet025', # backbone名称
'device_target': "GPU", # 训练平台 可选CPU、GPU、Ascend
'mode': 'Graph', # 模式，可选择动态图模式（'Pynative'）或者静态图模式('Graph')
'variance': [0.1, 0.2],# 方差
'clip': False, # 是否使用梯度裁剪
'loc_weight': 2.0, # Bbox回归损失权重
'class_weight': 1.0, # 置信度/类回归损失权重
'landm_weight': 1.0, # landmark回归损失权重
'batch_size': 8, # 训练批次大小
'num_workers': 1, # 数据集加载数据的线程数量
'num_anchor': 16800,# anchor数量
'ngpu': 1,# 训练的GPU数量
'nnpu': 1,# 训练的NPU数量
'image_size': 640, # 训练图像大小
'in_channel': 32, # FPN输入通道数
'out_channel': 64, # FPN输出通道数
...
'pretrain': False, # backbone是否加载预训练模型
...
'val_origin_size': False,# 是否使用图像原尺寸验证，选择True,则使用设定好的尺寸推理
'val_confidence_threshold': 0.02,# 置信度阈值
'val_nms_threshold': 0.4,# NMS阈值
'val_iou_threshold': 0.5,# IOU阈值
