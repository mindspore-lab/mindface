...
def calculate_accuracy(threshold, dist, actual_issame):
    ...
    t_p = np.sum(np.logical_and(predict_issame, actual_issame))
    f_p = np.sum(np.logical_and(predict_issame, np.logical_not(actual_issame)))
    t_n = np.sum(
        np.logical_and(np.logical_not(predict_issame),
                       np.logical_not(actual_issame)))
    f_n = np.sum(np.logical_and(np.logical_not(predict_issame), actual_issame))

    tpr = 0 if (t_p + f_n == 0) else float(t_p) / float(t_p + f_n)
    fpr = 0 if (f_p + t_n == 0) else float(f_p) / float(f_p + t_n)
    acc = float(t_p + t_n) / dist.size
    return tpr, fpr, acc
...
def test(data_set, backbone, batch_size, nfolds=10):
    ...
    for data in data_list:
        ...
        while b_a < data.shape[0]:
            ...
            embeddings[b_a:b_b, :] = _embeddings[(batch_size - count):, :]
            b_a = b_b
